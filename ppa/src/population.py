...
class Population:
    ...
    def mutate_offspring(self, offspring_points, generation, first_mutation_prob, second_mutation_prob, third_mutation_prob):
        # 2(r −0.5)(1−F(xi))
        ...
        # print(f"DOING {first_mutation_prob}:{first_prob <= first_mutation_prob} {second_mutation_prob}:{second_prob <= second_mutation_prob} {third_mutation_prob}:{third_prob <= third_mutation_prob}")
        # if (first_prob <= first_mutation_prob):
            # mutated_points = self.do_first_mutation(mutated_points, generation)
        mutated_points = self.do_first_mutation(mutated_points, first_mutation_prob)
            
        if (second_prob <= second_mutation_prob):
            mutated_points = self.do_second_mutation(mutated_points)
            did_second_mutation = True
        if (third_prob <= third_mutation_prob):
    ...
    def do_first_mutation(self, offspring_1_points, first_mutation_prob):
        ...
        for point in offspring_1_points:
            ...
            #     continue
            if random.uniform(0, 1) > first_mutation_prob:
                mutated_offspring_1_points.append(point)
                continue
            ...
            if is_corner_point:
                ...
                max_move_range = (avg_terminal_distance * first_mutation_prob)
